...
let STATIC_FILLS = new Map();  // will hold indeces for static filler items
...
function setter(match, name) {
  let options = FILLERS[name];
  if (options) {
    if(!STATIC_FILLS.has(name)){
      STATIC_FILLS.set(name, Math.floor(Math.random() * options.length));
    }
    return options[STATIC_FILLS.get(name)];
  } else {
    return `<UNKNOWN:${name}>`;
  }
}
...
function generate() {
  ...
  STATIC_FILLS.clear();  // clear static fill indeces to prevent sticky fill on reroll
  while (story.match(slotPattern)) {
    story = story.replace(slotPattern, replacer);
  }
  while (story.match(staticSlotPattern)) {
    story = story.replace(staticSlotPattern, setter);
  }
  /* global box */
  $("#box").text(story);
}
